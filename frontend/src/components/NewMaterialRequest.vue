<template>
	<BaseLayout :pageTitle="pageTitle" :home="'shopping-bag'" @cartIconClick="handleCartIconClick"   class="text-xs">
		<template #body>
          ...
          <datalist id="searchWarehouse">
            <option v-for="whName in warehouseOptions" :key="whName" :value="whName" >
              {{ whName }}
                ...
                <button  id="submitPurchaseReceipt"  @click="submitMaterialRequest">Submit</button>
				<button id="savePurchaseReceipt"  @click="saveMaterialRequest">Save</button>
...
const selectedPurpose = ref('Material Transfer');
let draftNum = null; 
...
//  window.onload()
	...
	try {
		const response = await axios.get('/api/method/wms.api.my_api.get_material_by_name', {
      ...
      if (response.data && response.data.message) {
            ...
            selectedPurpose.value = apiData.value.material_request_type || '';
            selectedDate.value = apiData.value.schedule_date || '';
...
const saveMaterialRequest = async () => {
  ...
  const requestData = {
    ...
    material_request_type: selectedPurpose.value,
  ...
  try {
    if (pageTitle.value && pageTitle.value !== "New") {
      ...
      const updateResponse = await axios.put(`/api/resource/Material Request/${pageTitle.value}`, requestData, {
        headers: { 'Authorization': `Bearer ${sid}` }
      });

      console.log('Updated Material Request', updateResponse.data);
      alert('Updated Material Request: ' + updateResponse.data.data.name);
    } else {
      
      const createResponse = await axios.post('/api/resource/Material Request', requestData, {
        headers: { 'Authorization': `Bearer ${sid}` }
      });

      console.log('Created Material Request', createResponse.data);
      pageTitle.value = createResponse.data.data.name;
      alert('Created Material Request: ' + pageTitle.value);
    }
  } catch (error) {
    console.error('Error in Material Request process:', error);
    try {
