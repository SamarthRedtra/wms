<template>
	<BaseLayout pageTitle="Material Request" :home="'home'" @cartIconClick="handleCartIconClick"   >
		<template #profileButton>
			
			<router-link :to="{ name: 'NewMaterialRequest' }" class="flex flex-col items-center">
					...
					<div class="flex flex-col bg-white rounded cursor-pointer">
						...
						>
							<div class="flex flex-row items-center gap-3 grow">
								...
								<FeatherIcon name="shopping-bag" class="h-6 w-6 text-black-500" />
                               
                                								<div id="fontChange" class=" font-normal text-gray-800 leading-6">
									<p >{{ link.name }} : <span>{{ link.set_warehouse }}</span></p>
...
const fetchDataII = async () => {
	const response = await axios.get('/api/method/wms.api.my_api.get_material_request_list')
...
const redirectToForm = (link) => {
  router.push({ name: 'NewMaterialRequest', query: { linkName: link.name } });
