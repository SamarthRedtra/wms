<template>
	<BaseLayout pageTitle="Bin Inquiry" :home="'home'" @cartIconClick="handleCartIconClick"   class="text-xs">
  ...
  <div id="openItemDropdown" class="flex flex-col items-center my-7 p-4 gap-7" >
    <div id="scanInput" class="relative" >
      <input
        ...
        v-model="receiptName"
        id="scanReceiptInput"
        placeholder="Type Warehouse and press enter"
      ...
      <div v-if="showItemDropdown" class="warehouse-dropdown" >
    ...
    <div id="reader" v-show="showScanner" @click="closeScanner"></div>
                ...
                >
                  <div class=" grow cursor-pointer" >
                   ...
                   <div id="enterData" class=" flex justify-between text-xs font-normal text-black-800 leading-6">
                    <p>itemCode</p>
                   <input readonly v-model="selectedItemCode" id="selectItemCode" />

                    <!-- <option v-for="link in itemCode" :key="link.name" :value="link.name">{{link.name}}</option> -->
          ...
          id="footTab"
          slot="bottom"
          class="w-full bg-white flex justify-around  sm:w-96 py-2 standalone:pb-safe-bottom"
        >
        <div class="bottomTab">
        ...
        <router-link class="bottomTab" to="ItemInquiry">
        <div >
...
const requestValue = errorMessage.value || receiptName.value
	...
	try {
		const purchaseReceipts = await axios.get('/api/method/wms.api.my_api.item_bin_data', {
      
      params: {
       "warehouse": requestValue
      } 
    });
...
const selectLink = (link) => {
  selectedItemCode.value = link.item_code;
...
const saveReconsillation =  async() => {
  ...
  const item_code = selectedItemCode.value;
