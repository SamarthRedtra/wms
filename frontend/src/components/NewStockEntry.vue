<template>
	<BaseLayout :pageTitle="pageTitle" :home="'layers'" @cartIconClick="handleCartIconClick"   class="text-xs">
		<template #body>
			<div  class="flex flex-col items-center">
 <div @click="handleClick" id="containerIII" class="flex justify-between text-base leading-6  rounded w-full py-1 px-4 border-none mt-5">

<p>Stock Entry Type</p>
	...
	<datalist id="searchWarehouse">
		<option v-for="whName in warehouseOptions" :key="whName" :value="whName" >
			{{ whName }}
                    ...
                    <label for="itemName">Target Warehouse</label>
<div class="barcode-input">
	<input   placeholder="Search item by warehouse" list="searchWarehouse" id="wareHouseName" v-model="selectedWarehouseII"/>
	<datalist id="searchWarehouse">
		<option v-for="whName in warehouseOptions" :key="whName" :value="whName" >
			{{ whName }}
		  </option>
                ...
                <button  id="submitPurchaseReceipt"  @click="submitStockEntry">Submit</button>
...
stName.value = route.query.stName;
console.log('stName', stName.value)
...
const editSummaryItem = (item) => {
  ...
  selectedWarehouseII.value = item.twarehouse;
...
const warehouselist = createListResource({
  doctype: 'Warehouse',
  fields: ['*'], 
  auto: true,
});
...
const addToSummary = () => {
    ...
    const twarehouse = selectedWarehouseII.value;
	const quantity = Number(selectedQuantity.value);

	
	const existingItem = summaryItems.value.find(item => item.itemCode === itemCode && item.warehouse === warehouse && item.twarehouse === twarehouse);
	...
	if (existingItem) {
		...
		summaryItems.value.push({ itemCode, warehouse, twarehouse, quantity });
	}
...
const canSubmit = () => {
  ...
  return selectedItemCode.value.trim() !== '' &&
         ...
         selectedWarehouseII.value.trim() !== '' &&
  ...
  if (canSubmit()) {
    if (currentItem.value.itemCode) {
      ...
      const existingItem = summaryItems.value.find(
        (i) => i.itemCode === currentItem.value.itemCode && i.warehouse === currentItem.value.warehouse && i.twarehouse === currentItem.value.twarehouse
      );
      if (existingItem) {
        existingItem.quantity = selectedQuantity.value;
        
        existingItem.warehouse = selectedWarehouse.value;
        existingItem.twarehouse = selectedWarehouseII.value
...
//  window.onload()
	...
	try {
		const response = await axios.get('/api/method/wms.api.my_api.get_stockentry_by_name', {
			params: { name: stName.value }
		});
      ...
      if (response.data && response.data.message) {
            ...
            selectedPurpose.value = apiData.value.stock_entry_type || '';
...
onMounted(() => {
  ...
  const stName = route.query.stName;
  if (stName) {
  
  
      fetchReceiptData(stName);
...
const saveStockReconcialation = async () => {
  ...
  try {
    if (pageTitle.value && pageTitle.value !== "New") {
      ...
      const updateResponse = await axios.put(`/api/resource/Stock Entry/${pageTitle.value}`, requestData, {
        headers: { 'Authorization': `Bearer ${sid}` }
      });

      console.log('Updated Stock Entry', updateResponse.data);
      alert('Updated Stock Entry: ' + updateResponse.data.data.name);
    } else {
      
      const createResponse = await axios.post('/api/resource/Stock Entry', requestData, {
        headers: { 'Authorization': `Bearer ${sid}` }
      });

      console.log('Created Stock Entry', createResponse.data);
      pageTitle.value = createResponse.data.data.name;
      alert('Created Stock Entry: ' + pageTitle.value);
    }
  } catch (error) {
    console.error('Error in Stock Entry process:', error);
    try {
