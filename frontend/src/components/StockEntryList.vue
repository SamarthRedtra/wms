<template>
	<BaseLayout pageTitle="Stock Entry" :home="'home'" @cartIconClick="handleCartIconClick"   >
		<template #profileButton>
			
			<router-link :to="{ name: 'NewStockEntry' }" class="flex flex-col items-center">
					...
					<div class="flex flex-col bg-white rounded cursor-pointer">
						...
						>
							<div class="flex flex-row items-center gap-3 grow">
								...
								<FeatherIcon name="layers" class="h-6 w-6 text-black-500" />
                               
                                								<div id="fontChange" class=" font-normal text-gray-800 leading-6">
									<p >{{ link.name }} : <span>{{ link.company }}</span></p>
									
									<p >{{link.modified}}</p>
								</div>
								<div id="cir">{{ link.status }}
...
const fetchDataII = async () => {
	const response = await axios.get('/api/method/wms.api.my_api.get_all_Stock_Entry')
...
const redirectToForm = (link) => {
  router.push({ name: 'NewStockEntry', query: { stName: link.name } });
