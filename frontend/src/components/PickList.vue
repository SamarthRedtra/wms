<template>
	<BaseLayout pageTitle="Pick List" :home="'home'" @cartIconClick="handleCartIconClick"   >
		...
		<template #body>
			<div class="flex flex-col items-center my-7 p-4 gap-7">
				...
				<div class="w-full">
					<TabButtons
					  :buttons="[{ label: 'Pending Picklist' }, { label: 'Completed Picklist' }]"
					  v-model="activeTab"
					  @tab-clicked="handleTabClicked"
			  
			  
					/>
					<RequestList v-if="activeTab === 'Pending Picklist'"  />
					<RequestList
					  v-else-if="activeTab === 'Completed Picklist'"
...
import axios from "axios"
  ...
  const activeTab = ref("Pending Picklist"); 
...
const fetchDataII = async () => {
	const response = await axios.get('/api/method/wms.api.my_api.get_all_pick_list')
